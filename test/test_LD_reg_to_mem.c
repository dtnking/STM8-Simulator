...
/*    Load byte from register to memory
**  =======================================
**   |   Name   : shortMem |
...
/*   Load byte from shortoff_X to memory
** =========================================
**   |   Name   : ShortOff X      |
...
/*   Load byte from shortoff_Y to memory
** =========================================
**   |   Name   : ShortOff Y      |
...
/*   Load byte from longoff_X to memory
** =========================================
**   |   Name   : LongOff X        |
...
/*   Load byte from longoff_Y to memory
** =========================================
**   |   Name   : LongOff Y        |
...
/*   Load byte from shortoff_SP to memory
** =========================================
**   |   Name   : ShortOff SP      |
...
/*         Load byte from shortptr_W to memory
** =======================================================
**   |   Name   : Shortptr W                               |
...
void test_LD_mem_to_reg_shortptr_W_0x11_expected_0x11(void){
  uint8_t instrc[]	= {0xc6,0x40};                       // Opcode : 0xc640
	...
	memory[0x41]			= 0xe5;
	memory[0x42e5]		= 0x11;
	ldMemToReg_shortptr_w(instrc);                                // store 0x11 into memory location 0x42e5.

	TEST_ASSERT_EQUAL_HEX8 (0x11, cpuRegisters->A);
}

/*         Load byte from longptr_W to memory
** ===========================================================
**   |   Name   : Longptr W                                   |
**   | Location : (0x1040 hold 0x42)+(0x1040+1 hold 0xe5)     |       [Accumulator]
**   |                          [^--(combine)-------^]=0x42e5 |            ^
**   |  Value   : 0x13                                        |--(load)----|
*/
void test_LD_mem_to_reg_longptr_W_0x13_expected_0x13(void){
  uint8_t instrc[]	= {0xc6,0x10,0x40};                   // Opcode : 0xc61040
	memory[0x1040]		= 0x42;
	memory[0x1041]		= 0xe5;
	memory[0x42e5]		= 0x13;                               // store 0x13 into memory location 0x42e5.
...
/*         Load byte from shortptr_W_X to memory
** =======================================================================
**   |   Name   : shortptr W X                                           |
**   | Location : (0x89 hold 0x08)+(0x89+1 hold 0x01)                    |       [Accumulator]
...
void test_LD_mem_to_reg_shortptr_W_X_0x40_expected_0x40(void){
  ...
  uint8_t instrc[] = {0xd6,0x89};                   // Opcode : 0xd689
	memory[0x89]		 = 0x08;
	memory[0x8a]		 = 0x01;
	memory[0x0804]	 = 0x40;                          // store 0x40 into memory location 0x0804.
	ldMemToReg_shortptr_w_X(instrc);

	TEST_ASSERT_EQUAL_HEX8 (0x40, cpuRegisters->A);
}

/*         Load byte from longptr_W_X to memory
** ==========================================================================
**   |   Name   : Longptr W X                                                |
**   | Location : (0x1089 hold 0x18)+(0x1089+1 hold 0x01)                    |       [Accumulator]
...
void test_LD_mem_to_reg_longptr_W_X_0x42_expected_0x42(void){
  ...
  uint8_t instrc[] = {0xd6,0x10,0x89};              // Opcode : 0xd61089
	memory[0x1089]	 = 0x18;
	memory[0x108a]	 = 0x01;
	memory[0x1811]	 = 0x42;                          // store 0x42 into memory location 0x1811.
	ldMemToReg_longptr_w_X(instrc);

	TEST_ASSERT_EQUAL_HEX8 (0x42, cpuRegisters->A);
}

/*         Load byte from shortptr_W_Y to memory
** ========================================================================
**   |   Name   : shortptr W Y                                            |
**   | Location : (0x89 hold 0x08)+(0x89+1 hold 0x01)                     |       [Accumulator]
